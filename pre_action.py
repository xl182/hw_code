...
def allocate_spaces(
    ...
    g.public_bounds = [
        [[0, 0] for j in range(len(assignments[i]))] if i != 0 else [[-1]]
        for i in range(len(assignments))
    ]
    ...
    for i in range(1, len(assignments)):
        ...
        for j in range(1, len(assignments[i])):
            ...
            g.left_allocate_sizes[i] += min_cost[
                tag
            ]  # record write bounds sizes of each volume
            g.write_bounds[tag][copy_serial[tag]][1] = volume_positions[i]

            if (
                g.public_sizes[i][j] > 0
...
def pre_action():
    ...
    init_variables(g.T, g.M, g.N, g.V, g.G, g)
    ...
    while True:
        g.disk_assignments, split_files = allocate_files(
            finnal_cost_list, g.N, g.V
        )  # vloume
