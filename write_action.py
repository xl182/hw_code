...
from get_in import write_input
from utils import log, log_disk, sys_break
...
def write_method_3(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    for i in range(len(g.empty_spaces[obj_tag]) - 1, obj_size, -1):
        ...
        if copys[-1][1]:
            if g.use_write_log:
                log(
                    f"current id: {obj_id}, current size: {obj_size}, current tag: {obj_tag}"
                )
...
def write_method_4(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    if g.use_write_log:
        ...
        log(
            f"obj_tag: {obj_tag}, obj id: {obj_id}, obj_size: {obj_size} left copy num: {left_copy_num}"
        )
    ...
    for i in g.tag_assignments[obj_tag][1:]:
        ...
        if g.use_write_log:
            log(
                f"index: {i}, {g.public_bounds[i][assign_pos]}: {g.public_bounds[i][assign_pos-1]}, "
            )

        if (
            g.public_bounds[i][assign_pos][0] + obj_size
            <= g.public_bounds[i][assign_pos][1]
        ):
            ...
            g.public_bounds[i][assign_pos - 1][0] + obj_size
            <= g.public_bounds[i][assign_pos - 1][1]
        ):
...
def write_method_5(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    for i in range(1, g.N + 1):
        ...
        index, position = -1, -1
        if (
            g.write_bounds[left_tag][left_copy_serial][0] + obj_size
            <= g.write_bounds[left_tag][left_copy_serial][1]
        ):
            ...
            <= g.write_bounds[right_tag][right_copy_serial][1]
        ):
        ...
        if left_copy_num == 0:
            if g.use_write_log:
                log(f"5 left_copy_num: {left_copy_num}")
            return left_copy_num
...
def write_method_6(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    top_indices = sorted(
        range(len(g.left_allocate_sizes)),
        key=lambda i: g.left_allocate_sizes[i],
        reverse=True,
    )
    if g.use_write_log:
        log(f"obj_id: {obj_id}")
    ...
    for i in top_indices:
        ...
        for t, tag in enumerate(tag_list[1:]):
            ...
            diff_list[t + 1] = (
                g.write_bounds[tag][tag_copy_serial][1]
                - g.write_bounds[tag][tag_copy_serial][0]
            )
...
def write_method_7(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    public_bounds_sizes = [
        [0 for j in range(len(g.public_bounds[i]))] for i in range(g.N + 1)
    ]
    public_bounds_size_sum = [0 for i in range(g.N + 1)]
    for i in range(1, g.N + 1):
        for j in range(1, len(g.public_bounds[i])):
            public_bounds_sizes[i][j] += (
                g.public_bounds[i][j][-1] - g.public_bounds[i][j][0]
            )
        public_bounds_size_sum[i] = sum(public_bounds_sizes[i])
    top_indices = sorted(
        range(1, g.N + 1), key=lambda i: public_bounds_size_sum[i], reverse=True
    )
...
def write_method_8(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    for t in range(1, 1 + g.M):
        ...
        copy_serial = g.COPY_NUM - left_copy_num + 1
        for s in range(g.tag_max_obj_size[t], obj_size, -1):
            ...
            for c in range(1, g.COPY_NUM + 1):
                ...
                if left_copy_num == 0 or index in wo.disk_serial:
                    g.free_empty_spaces[index][copys[c - 1][1]].append(copys[c - 1])  # type: ignore
                    continue
...
def do_write_object(obj_id, obj_size, obj_tag):
    ...
    if g.use_write_log:
        log(
            f"write id: {obj_id}, write size: {obj_size}, write tag: {obj_tag}, index: {wo.disk_serial}, position: {wo.position}"
        )
