...
def write_method_2(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    for c in range(1, COPY_NUM+1):
        ...
        left_copy_num -= 1
        if left_copy_num == 0:
            if use_write_log and use_method_2:
...
def write_method_3(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    if use_method_3 and use_write_log:
        log("write_method 3")
...
def write_method_5(left_copy_num, obj_id, obj_size, obj_tag, wo):
    ...
    for i in range(1, N + 1):
        ...
        left_copy_num -= 1
        if left_copy_num == 0:
            if use_write_log: log(f"5 left_copy_num: {left_copy_num}")
            return left_copy_num
...
def timestamp_action():
    global current_timestamp, use_write_log
...
def pre_action():
    ...
    while True:
        ...
        if max(disk_cost) > V:
            disk_assignments = tmp_disk_assignments
            break
        if use_write_log: log(f"try disk_cost: {disk_cost}")
